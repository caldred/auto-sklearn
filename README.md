...
# 3. Fit and predict
training_run = fit(graph, X_train, y_train, config)
predictions = training_run.predict(X_test)
```

This tunes a random forest over 50 Optuna trials using 5-fold stratified CV. At inference time, predictions from all fold models are averaged.

Note: `fit()` accepts raw `X`/`y` (DataFrames/arrays) or a pre-built `DataView` for advanced use cases. The score direction (`greater_is_better`) is inferred automatically for standard sklearn metrics like `roc_auc`, `accuracy`, and `neg_mean_squared_error`.

## Core Concepts

sklearn-meta has three phases:

1. **Graph definition** -- `GraphBuilder` produces an immutable `GraphSpec` describing model nodes and their dependencies (stacking, distillation, transforms). No runtime config here.
2. **Configuration** -- `RunConfigBuilder` produces a `RunConfig` specifying how to train: CV strategy, number of tuning trials, feature selection, etc.
3. **Execution** -- `fit()` orchestrates the pipeline: tune hyperparameters, select features, train fold models, and return a `TrainingRun`.

This separation means you can test the same graph with different configs, or different graphs with the same config.
...
4. [Configuration Reference](#configuration-reference)
   ...
   - [Estimator Scaling](#estimator-scaling)
5. [Advanced](#advanced)
   - [Knowledge Distillation](#knowledge-distillation)
   - [Joint Quantile Regression](#joint-quantile-regression)
   - [Low-Level API](#low-level-api)
   - [Custom Search Backend](#custom-search-backend)
   - [Fit Caching](#fit-caching)
   - [Plugins](#plugins)
...
Tune a single model with cross-validation:
...
training_run = fit(graph, X_train, y_train, config)
predictions = training_run.predict(X_test)
print(f"Best params: {training_run.node_results['gb'].best_params}")
print(f"CV score: {training_run.node_results['gb'].mean_score:.4f}")
```

### Model Stacking

Build multi-layer stacking ensembles. Upstream model predictions become features for downstream models, with automatic out-of-fold handling to prevent data leakage:
...
Automatically drop uninformative features before final training. Configure feature selection on the `RunConfigBuilder`:
...
sklearn-meta uses `DataView` as a lazy, immutable view over datasets. For simple cases, `fit()` accepts raw DataFrames directly. For advanced use, construct a `DataView` explicitly.
...
# fit() wraps X/y into a DataView internally
training_run = fit(graph, X_train, y_train, config)
training_run = fit(graph, X_train, y_train, config, groups=patient_ids)
```
...
Hyperparameter search spaces are defined using builder methods on `NodeBuilder`. `.param()` infers integer ranges from integer bounds, treats `0..1` as a float range, and also accepts a list for categorical parameters:

```python
.add_model("xgb", XGBClassifier)
.param("n_estimators", 50, 500)               # Integer range
.param("max_depth", 3, 15)                    # Integer range
.param("learning_rate", 0.001, 0.3, log=True)  # Float range, log-uniform
.param("subsample", 0, 1)                      # Float range (unit interval)
.param("booster", ["gbtree", "dart"])           # Categorical (list shorthand)
```

The builder methods:
- `.param(name, low, high)` infers `int` when both bounds are integers, except `0..1`, which stays float. Add `log=True` or `step=` as needed. Pass a list instead for categorical.
...
.tuning(
    ...
    metric="roc_auc",              # Any sklearn scorer name
...
`greater_is_better` is inferred automatically for all standard sklearn scorer names. All sklearn scorers follow the higher-is-better convention (loss metrics like `neg_mean_squared_error` are already negated). You only need to set it explicitly for custom metrics:

```python
# Inferred automatically:
.tuning(metric="roc_auc")                     # inferred: True
.tuning(metric="neg_mean_squared_error")       # inferred: True (already negated)
.tuning(metric="accuracy")                     # inferred: True

# Custom metrics need explicit setting:
.tuning(metric="my_custom_loss", greater_is_better=False)
```
...
`fit()` returns a `TrainingRun` containing all trained models and metadata.
...
# Quick prediction (compiles InferenceGraph internally, caches it)
predictions = training_run.predict(X_test)
probabilities = training_run.predict_proba(X_test)

# Or compile explicitly for repeated use or saving
...
predictions = loaded_run.predict(X_test)
```
